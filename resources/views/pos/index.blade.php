<x-app-layout>
    ...
    <script>
        function pos() { return {
            query: '', results: [], searched: false, searchSequence: 0, scanGeneration: 0, scanQueue: [], scanProcessing: false, cart: @js($initialCart), payment: 'cash', cashReceived: '', cashAmount: '', qrAmount: '', submitting: false, clearModalOpen: false, clearingCart: false, raffleTickets: @js($initialRaffleQuote), raffleDecision: '', raffleModalOpen: false, customerMode: 'search', customerQuery: '', customerResults: [], customerSearched: false, selectedCustomer: null, customerFullName: '', customerPhone: '', customerCi: '', customerFormError: '',
            async fetchProducts(term, sequence) { const response = await fetch(`{{ route('pos.products.search') }}?search=${encodeURIComponent(term)}`, {headers: {'Accept': 'application/json'}}); const products = response.ok ? await response.json() : []; return sequence === this.searchSequence ? products : null; },
            async search() { const term = this.query.trim(); const sequence = ++this.searchSequence; this.searched = term.length > 0; if (!this.searched) { this.results = []; return; } const products = await this.fetchProducts(term, sequence); if (products !== null) this.results = products; },
            handleSearchEnter() { const term = this.query.trim(); if (!term) return; const generation = ++this.scanGeneration; ++this.searchSequence; this.query = ''; this.results = []; this.searched = false; this.scanQueue.push({term, generation}); this.$nextTick(() => this.$refs.productSearch.focus()); this.processScanQueue(); },
            async processScanQueue() { if (this.scanProcessing) return; this.scanProcessing = true; try { while (this.scanQueue.length) { const scan = this.scanQueue.shift(); const response = await fetch(`{{ route('pos.products.search') }}?search=${encodeURIComponent(scan.term)}`, {headers: {'Accept': 'application/json'}}); const products = response.ok ? await response.json() : []; const exactMatches = products.filter(product => product.exact_code_match); if (exactMatches.length === 1) await this.add(exactMatches[0], true); else if (scan.generation === this.scanGeneration && this.query === '') { this.query = scan.term; this.results = products; this.searched = true; } } } finally { this.scanProcessing = false; this.$nextTick(() => this.$refs.productSearch.focus()); } },
            async add(product, preserveSearch = false) { let item = this.cart.find(i => i.id === product.id); if (item) item.quantity = String(Number(item.quantity) + 1); else { item = {...product, quantity: '1', observed_price: product.price, price_changed: false, available: true, error: ''}; this.cart.push(item); } if (!preserveSearch) { this.query = ''; this.results = []; this.searched = false; this.searchSequence++; } this.validateItem(item); if (!item.error) await this.sync(item); this.$nextTick(() => this.$refs.productSearch.focus()); },
