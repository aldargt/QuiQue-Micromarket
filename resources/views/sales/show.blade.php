<x-app-layout>
    <x-slot name="header"><div class="flex flex-wrap items-center justify-between gap-3"><h1 class="text-2xl font-bold tracking-tight text-gray-900">Venta {{ $sale->sale_number }}</h1><div class="flex flex-wrap items-center gap-3"><a href="{{ route('sales.index') }}" class="ui-link text-sm">Volver al historial</a>@if ($sale->status === App\Enums\SaleStatus::Confirmed)<a href="{{ route('sales.receipt', $sale) }}" class="ui-button-secondary"><svg class="me-2 h-5 w-5" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M7 8V4h10v4M7 17H5a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2M7 14h10v6H7v-6Z"/><path stroke-linecap="round" d="M17 11h.01"/></svg>Imprimir recibo</a>@endif<a href="{{ route('pos.index') }}" class="ui-button-primary">Nueva venta</a></div></div></x-slot>
        ...
        @if ($sale->status === App\Enums\SaleStatus::Confirmed && $sale->confirmed_at->timezone(config('app.timezone'))->isSameDay(now(config('app.timezone'))))
            ...
            <div x-cloak x-show="cancelOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 p-4" role="dialog" aria-modal="true">
                <form method="POST" action="{{ route('sales.cancel', $sale) }}" class="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">@csrf
                    ...
                    <div class="mt-5"><x-input-label for="reason" value="Motivo de anulación" required/><textarea id="reason" name="reason" rows="4" maxlength="500" required class="mt-1 block w-full rounded-md border-gray-300" placeholder="Ej.: Producto devuelto por el cliente">{{ old('reason') }}</textarea><x-input-error :messages="$errors->get('reason')" class="mt-2"/></div>
